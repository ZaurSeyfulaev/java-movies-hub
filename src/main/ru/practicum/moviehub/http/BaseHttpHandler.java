...
abstract class BaseHttpHandler implements HttpHandler {
    ...
    public static class MoviesHandler extends BaseHttpHandler { // Расширьте базовый класс BaseHttpHandler
        ...
        private void handleGet(HttpExchange ex, String path, String query) throws IOException {
            ...
            if (Pattern.matches("^/movies$", path) && query.contains("year=")) {
                ...
                if (filtered.isEmpty()) {
                    sendError(ex, 404, "Фильм не найден", List.of("В коллекции нет фильмов с указанным годом"));
                    return;
                }
